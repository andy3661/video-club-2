...
<form  action="{{route('rent.update')}}" method="post" class="row g-3">
  @method('PUT')
    @csrf()
    <div class="col-md-12">
        <div class="row">
 ...
 @endphp
  ...
  $cont=$cont+1;
  @endphp
  @if(count($selected_films)>0)
  @if($selected_films[$cont]->id == $film->id)
  <option  selected value="{{$film->id.'|'.$film->unitary_price.'|'.$film->type}}">{{$film->name}}</option>
  @else
...
<input class="form-control" name="start_date" type="datetime-local" id="start_date" value="{{$rent->start_date}}">
  </div>
  ...
  <option selected value="{{$clients->id}}">{{$clients->name}}</option>
  @endif
  ...
  <div class="col-md-12">
    ...
    <input name="total_value" readonly type="text" class="form-control" id="total_value" placeholder="$" value="{{$rent->total_value}}">
  </div>
  <div class="col-md-12">
  <label for="end_date">Fecha de finalizacion:</label>

<input class="form-control" name="end_date" type="datetime-local" id="end_date" value="{{$rent->end_date}}"
      >
  </div>
        </div>
